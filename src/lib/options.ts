...
export async function getOption(
  ...
  try {
    ...
    console.warn(`[options] Failed to fetch option "${name}", using default:`, error instanceof Error ? error.message : error);
    return defaultValue;
  }
}
...
export async function getOptions(
  ...
  if (toFetch.length > 0) {
    try {
      ...
      console.warn(`[options] Failed to fetch options, using defaults:`, error instanceof Error ? error.message : error);
    }
